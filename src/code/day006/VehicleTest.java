...
public class VehicleTest {
    ...
    public static void removeVehicleMsgByVid() {
        // 输入车辆编号
        ...
        if (isIdExist(vid)) {
            ...
            VehicleUtils.printInfo(deleteVehicleByVid(vid) ? "删除车辆信息成功" : "删除失败,请重试!");
        } else {
            // 如果编号不存在,提示提示信息.
    ...
    public static void showVehiclesFromType() {
        VehicleUtils.printInfo("请输入要查询详细车辆信息的类型:");
        String vType = scan.next();
        // 调用查询方法查询所有车辆信息保存到变量vehicleArr中
        Vehicle[] vehiclesArr = queryVehiclesByMiddleSize(vType);
        // 判断是否查询到数据
    ...
    public static void showVehiclesAvgFromStructure() {
        VehicleUtils.printInfo("请输入要查询的结构车型的平均值");
        String vStructure = scan.next();

        // 调用方法计算的平均价格
        double suvAvg = calculateAvgByStructure(vStructure);
    ...
    public static void showVehiclesMaxCostFromBrand() {
        VehicleUtils.printInfo("请输入要查询最高价的品牌");
        String vBrand = scan.next();
        Vehicle[] vehicle = findMaxCostFromBrand(vBrand);
    ...
    public static void showVehiclesCostThanTwentyThousand() {
        // 获取超过20万售价的汽车,存入变量中
        Vehicle[] vehicles = queryVehiclesByCost();
    ...
    public static void showVehicleMsgByVid() {
        // 提示用户输入信息,并接收编号信息
    ...
    public static void showVehiclesMsgFromKinds() {
        // 查询车辆品牌,记数,并存储相应信息.详细操作见 queryVehicleByKinds() 方法.
        List<Vehicle[]> veKindsArr = queryVehicleByKinds();
    ...
    public static void showDefaultMsg() {
